                        ...
                        <tbody>
                            @forelse ($choises as $choise)
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{ route('admin.choises.edit', ['id' => $choise->id]) }}" class="btn color-warning btn-sm mr-2"><i class="fas fa-pen"></i>  Редактировать</a>
                                            <form onsubmit="if(confirm('Удалить?')) {return true} else {return false}" action="{{route('admin.choises.destroy', $choise)}}" method="post">
                                                @csrf                         
                                                <input type="hidden" name="_method" value="delete">                         
                                                <button type="submit" class="btn btn-danger btn-sm"><i class="far fa-trash-alt"></i>  Удалить</button>                                                 
                                            </form>
                                        </div>
                                    </td>
                                </tr> 
